...
from data.models import FreeReducedLunchEligible
import csv
...
# 4) change 'amount' column in data_FreeReducedLunchEligible table to type 'bigint'
# 5) Run as Django management command from your project path "python manage.py import_free_reduced_lunch_eligible"

SOURCE_FILE = '%s/education/free_reduced_lunch_eligible.csv' % (settings.LOCAL_DATA_ROOT)
        ...
        for i, row in enumerate(data_reader):
            if i == 0:
                ...
                for j,col in enumerate(row):
                    if j > 2:
                        record = FreeReducedLunchEligible()
