...
# npp csv: http://assets.nationalpriorities.org/raw_data/education/hs_dropouts.csv (updated 7/21/2010)
...
SOURCE_FILE = '%s/education/hs_dropouts.csv' % (settings.LOCAL_DATA_ROOT)
            ...
            if i == 0:
                ...
                for j,col in enumerate(row):
                    if j == 0:
                        ...
                        year = year_row[j]
                        value = col
                        record = HighSchoolDropouts(state=state, year=year, value=clean_int(value))
                        record.save()